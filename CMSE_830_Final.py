...
df = pd.read_csv("E:/MSDS/CMSE830/CMSE_830_Project_Mid_Term/heart_2020_cleaned.csv")
...
def Introduction():
     ...
     st.image("https://github.com/vishalpanda10/cmsemidterm/blob/main/heart.jpg")
...
def Demographics():
    ...
    if features=='Smoking':
        ...
        st.write("Do people with heart disease smoke more?")
        st.pyplot(show_relation(obj_cols[0], 'HeartDisease', type_='count'))
        st.write("People who smoke are more susceptible to heart disease.")


        st.write("Are smokers satisfied with their health?")
        st.pyplot(show_relation('GenHealth', 'Smoking', 'count'))

        st.write("- People who are smoking said that they have good and very health.")


    if features=='AlcoholDrinking':
        st.write("Do people with heart disease consume alcohol?")
        st.pyplot(show_relation(obj_cols[1], 'HeartDisease', type_='count'))
        st.write("")

    if features=='Stroke':
        st.write("Does having a stroke affect the chances of heart disease?")
        st.pyplot(show_relation(obj_cols[2], 'HeartDisease', type_='count'))
        st.write("- People with stroke have higher chances of heart disease(It is highly correlated)")

    if features=='DiffWalking':
        st.write("Does having difficulty walking affect heart disease?")
        st.pyplot(show_relation(obj_cols[3], 'HeartDisease', type_='count'))

    if features=='Sex':

        st.write("Are males more likely to suffer from heart disease?")


        fig=show_relation(obj_cols[4], 'HeartDisease', type_='count')
        st.pyplot(fig)

        st.write("Males are more susceptible to the heart disease.")

    if features=='AgeCategory':

        st.write("Are older individuals more susceptible to heart disease?")

        fig1=show_relation(obj_cols[5], 'HeartDisease', type_='count')
        st.pyplot(fig1)
        ...
        st.write("""
        - Age plays a big factor in heart disease.
        - The most susceptible are the age category above 70
        
        """)
    if features=='Race':

        st.write("Heart Disease among Races")
        st.pyplot(show_relation(obj_cols[6], 'HeartDisease', type_='count'))
        
        st.write("- Highest among Native Americans and lowest among Asians")
        st.write("- There is no correlation")
    ...
    if features=='Diabetic':
        st.write("Does being diabetic increase the chances of heart disease?")
        st.pyplot(show_relation(obj_cols[7], 'HeartDisease', type_='count'))
        
        
    if features=='PhysicalActivity':

        st.write(" Are heart disease patients less physically healthy or active?")
        st.pyplot(show_relation(obj_cols[8], 'HeartDisease', type_='count'))
        ...
        ax1.pie([x[0], x[1]], labels = ['Physical Activity', 'No physical activity'], colors=colors2, autopct='%.0f%%');

        ax1.set_title("Individuals Without Heart Disease");

        ax2.pie([x[2], x[3]], labels = ['Physical Activity', 'No physical activity'], colors=colors2, autopct='%.0f%%');
        ax2.set_title("Individuals With Heart Disease");

        st.pyplot(fig)
        st.write("- A larger percentage of heart disease patients are not physically active")

    if features=='GenHealth':

        st.write("General Health of Heart Disease Patients")
        st.pyplot(show_relation(obj_cols[9], 'HeartDisease', type_='count'))

        st.write("""
         - Most heart patients believe their health is good.
         - People who do not suffer have excellent health
         - A lot more who suffer have poor health.       
                """)
    ...
    if features=='Asthma':
        st.write("Do asthmatic people suffer more from heart diseases?")
        st.pyplot(show_relation(obj_cols[10], 'HeartDisease', type_='count'))

    if features=='KidneyDisease':
        st.write("Does kidney disease coincide with heart disease?")
        st.pyplot(show_relation(obj_cols[11], 'HeartDisease', type_='count'))

    if features=='SkinCancer':
        st.pyplot(show_relation(obj_cols[12], 'HeartDisease', type_='count'))
...
def Page4():
    heart=pd.read_csv("E:/MSDS/CMSE830/CMSE_830_Project_Mid_Term/heart_2020_ml.csv")
    ...
    features=st.multiselect("Number of Features",['BMI', 'Smoking', 'AlcoholDrinking', 'Stroke',
       ...
       'Asthma', 'KidneyDisease', 'SkinCancer'],['BMI','GenHealth','SleepTime','PhysicalHealth', 'MentalHealth'])
    ...
    st.write("# **Machine Learning Approaches**")
...
Consider a scenario where we need to classify whether an email is spam or not. If we use linear regression for this problem, there is a need for setting up a threshold based on which classification can be done. Say if the actual class is malignant, predicted continuous value 0.4 and the threshold value is 0.5, the data point will be classified as not malignant which can lead to serious consequence in real time.
        ...
        st.write("Accuracy:",round(accuracyscore,2),   "\tPrecision:",precision,"\tRecall:",recall,"\tF1 Score:",round(f1score,2))
...
Then it does the majority vote i.e the most common class/label among those K entries will be the class of the new data point.
    ...
    with tab4:
        ...
        precision=precision_score(y_test,pred)
        recall=recall_score(y_test,pred)
        st.write("Accuracy:",round(accuracyscore,3),   "\tPrecision:",round(precision,3),"\tRecall:",recall,"\tF1 Score:",round(f1score,2))
...
page=st.sidebar.radio(' ',['Introduction','Data','Demographics(Categorical)','Demographics(Numerical)','Insights&Summary',"Machine Learning Approaches"])
...
if page=='Insights&Summary':
       Page3()
if page=='Machine Learning Approaches':
       Page4()
